...
void Ticket::inputTicket() {
    ...
    do { std::cin >> strForChecking; } while (!std::regex_match(strForChecking, result, regular));
    for (j = 0; j < numbersOfAvailableSymbols; j++) { if (strForChecking[j] == 0) break; }
    j--;
    for (; j >= 0; j--) {
...
void Ticket::setStation1(basic_string<char> station1) {
    this->station1.assign(station1);
//    this->station1.copy(station1, strlen(station1), 0);
}

void Ticket::setStation2(basic_string<char> station2) {
    this->station2.assign(station2);
}

void Ticket::setDepDay(basic_string<char> depDay) {
    this->depDay.assign(depDay);
}

void Ticket::setDepTime(basic_string<char> depTime) {
...
Ticket::Ticket(int train, std::string &station1, std::string &station2, std::string &depDay, std::string depTime,
...
//Ticket::Ticket(Ticket& addressTicket){
//    this->train     = addressTicket.train;
//    this->station1  = addressTicket.station1;
//    this->station2  = addressTicket.station2;
//    this->depDay    = addressTicket.depDay;
//    this->depTime   = addressTicket.depTime;
//    this->coach     = addressTicket.coach;
//    this->seat      = addressTicket.seat;
//    this->price     = addressTicket.price;
//}
...
int Ticket::setAllInfoFromLine(std::string &line) {
    std::cout << "line.length: " + line.length() << std::endl;
    this->train = atoi(line.substr(0, 0 + 5).c_str());
    this->station1 = line.substr(5, numbersOfAvailableSymbols + 1);
    this->station2 = line.substr(21, numbersOfAvailableSymbols + 1);
    this->depDay = line.substr(37, 10);
    this->depTime = line.substr(47, 10);
    this->coach = atoi(line.substr(57, 4).c_str());
    this->seat = atoi(line.substr(62,  3).c_str());
    this->price = atoi(line.substr(65, 5).c_str());

    std::cout << "train: " << train << std::endl;
    std::cout << "station1: " << station1 << std::endl;
    std::cout << "station2: " << station2 << std::endl;
    std::cout << "depDay: " << depDay << std::endl;
    std::cout << "depTime: " << depTime << std::endl;
